...
public abstract class AbstractLmdbDb<K, V> implements LmdbDb {
    ...
    public boolean put(final K key, final V value, final boolean overwriteExisting) {
        try (final PooledByteBufferPair pooledByteBufferPair = getPooledBufferPair()) {
            final ByteBuffer keyBuffer = pooledByteBufferPair.getKeyBuffer();
            final ByteBuffer valueBuffer = pooledByteBufferPair.getValueBuffer();
            ...
            try (final Txn<ByteBuffer> writeTxn = lmdbEnvironment.txnWrite()) {
                final boolean didPutSucceed = put(writeTxn, keyBuffer, valueBuffer, overwriteExisting);
                if (didPutSucceed) {
                    writeTxn.commit();
                } else {
                    LOGGER.error("Unable to put search data to LMDB");
                    throw new RuntimeException(LogUtil.message("Error putting key {}, value {}", key, value));
                }
