...
public class ExpressionParser {
    ...
    public String[] getReferencedFieldNames(final String input) throws ParseException {
        final FieldNameIndex fieldNameIndex = new FieldNameIndex();
        parse(fieldNameIndex, input);
        return fieldNameIndex.getNames();
    }

    public Expression parse(final String[] fieldNames, final String input) throws ParseException {
        final FieldNameIndex fieldNameIndex = new FieldNameIndex(fieldNames);
        return parse(fieldNameIndex, input);
    }

    private Expression parse(final FieldNameIndex fieldNames, final String input) throws ParseException {
        ...
        List<Param> objects = new ArrayList<>(tokens.size());
        ...
        while (objects.size() != lastSize) {
            ...
            objects = processObjects(objects, fieldNames);
        }
    ...
    private List<Param> processObjects(final List<Param> objects, final FieldNameIndex fieldNames) throws ParseException {
        ...
        for (int i = 0; i < objects.size(); i++) {
            ...
            if (object instanceof Token) {
                ...
                if (Type.FUNCTION_START.equals(token.getType())) {
                    ...
                    final Function function = getFunction(objects, start, end, fieldNames);
        ...
        final Param param = getParam(copyList(objects, 0, objects.size() - 1), fieldNames);
        return Collections.singletonList(param);

    }

    private Function getFunction(final List<Param> objects, final int start, final int end, final FieldNameIndex fieldNames) throws ParseException {
        // Get the function.
        ...
        if (functionStart <= functionEnd) {
            ...
            int paramStart = -1;
            final List<Param> paramList = new ArrayList<>((functionEnd - functionStart) + 1);
            ...
            for (int i = functionStart; i <= functionEnd; i++) {
                ...
                if (object instanceof Token) {
                    ...
                    if (Type.COMMA.equals(token.getType())) {
                        ...
                        final Param param = getParam(copyList(objects, paramStart, paramEnd), fieldNames);
                        paramList.add(param);
            ...
            if (paramStart != -1) {
                final Param param = getParam(copyList(objects, paramStart, functionEnd), fieldNames);
                paramList.add(param);
            }

            // Turn param list into an array.
            params = paramList.toArray(new Param[0]);
    ...
    private Param getParam(final List<Param> objects, final FieldNameIndex fieldNames) throws ParseException {
        ...
        if (newObjects.size() == 1) {
            ...
            if (object instanceof Token) {
                final Token token = (Token) object;
                return paramFactory.create(fieldNames, token);
            }
            return object;
        }
        ...
        while (newObjects.size() > 1 && newObjects.size() != lastSize) {
            ...
            newObjects = applyBODMAS(newObjects, fieldNames);
        }
        ...
        while (newObjects.size() > 1 && newObjects.size() != lastSize) {
            lastSize = newObjects.size();
            newObjects = applyEquality(newObjects, fieldNames);
        }
        ...
        final Param object = newObjects.get(1);
        if (object instanceof Token) {
            final Token token = (Token) object;
    ...
    private List<Param> applyBODMAS(final List<Param> objects, final FieldNameIndex fieldNames) throws ParseException {
            ...
            for (int i = 0; i < objects.size(); i++) {
                ...
                if (object instanceof Token) {
                    ...
                    if (type.equals(token.getType())) {
                        ...
                        // Get left param.
                        final Param leftParam = getParam(copyList(objects, leftParamIndex, leftParamIndex), fieldNames);
                        // Get right param.
                        final Param rightParam = getParam(copyList(objects, rightParamIndex, rightParamIndex), fieldNames);
                        ...
                        if (leftParam == null) {
                            ...
                            if (Type.SUBTRACTION.equals(type)) {
                                ...
                                negate.setParams(new Param[]{rightParam});
                                param = negate;
                            }
                        } else {
    ...
    private List<Param> applyEquality(final List<Param> objects, final FieldNameIndex fieldNames) throws ParseException {
        ...
        for (final Type type : EQUALITY) {
            for (int i = 0; i < objects.size(); i++) {
                ...
                if (object instanceof Token) {
                    ...
                    if (type.equals(token.getType())) {
                        // Get before param.
                        final Param leftParam = getParam(copyList(objects, 0, i - 1), fieldNames);
                        // Get after param.
                        final Param rightParam = getParam(copyList(objects, i + 1, objects.size() - 1), fieldNames);
    ...
    private List<Param> copyList(final List<Param> list, final int startIndex, final int endIndex) {
        ...
        final List<Param> newList = new ArrayList<>((endIndex - startIndex) + 1);
    ...
    private List<Param> sandwich(final List<Param> list, final Param object, final int insertStart, final int insertEnd) {
        ...
        final List<Param> newList = new ArrayList<>(list.size() + 1);
