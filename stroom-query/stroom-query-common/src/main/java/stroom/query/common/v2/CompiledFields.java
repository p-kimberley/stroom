...
class CompiledFields implements Iterable<CompiledField> {
    private final List<CompiledField> compiledFields;

    CompiledFields(final List<Field> fields,
        ...
        if (null != fields) {
            ...
            for (final Field field : fields) {
                ...
                if (field.getExpression() != null && field.getExpression().trim().length() > 0) {
                    try {
                        expression = expressionParser.parse(fieldNames, field.getExpression());
