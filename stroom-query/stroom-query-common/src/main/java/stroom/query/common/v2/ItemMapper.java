...
//    }
    ...
    public void toKryo(final Val[] values,
                       final Kryo kryo,
                       final Output output) {
    ...
    public Generator[] fromKryo(final Kryo kryo,
                                final Input input) {
    ...
    public GroupKey[] createGroupKeys(final Generator[] generators) {
        ...
        final Object[] depthMap = new Object[compiledDepths.getMaxDepth()];
        ...
        for (int depth = 0; depth < fieldIndicesByDepth.length; depth++) {
            ...
            if (fieldIndexes != null) {
                for (final int fieldIndex : fieldIndexes) {
                    ...
                    List<Val> list = (List<Val>) depthMap[depth];
                    if (list == null) {
                        list = new ArrayList<>(fields.size());
                        depthMap[depth] = list;
                    }
                    list.add(generator.eval());
                }
            }
        }

        GroupKey[] groupKeys = new GroupKey[compiledDepths.getMaxDepth()];
        GroupKey parentKey = null;
        for (int depth = 0; depth < compiledDepths.getMaxDepth(); depth++) {
            List<Val> groupValues = null;
            if (depthMap.length > depth) {
                groupValues = (List<Val>) depthMap[depth];
            }

            final GroupKey key = new GroupKey(parentKey, groupValues);
            groupKeys[depth] = key;
            parentKey = key;
        }
