...
class TestTableDataFactory {
    ...
    void basicTest() {
        ...
        final TableDataFactory tableDataFactory = new TableDataFactory(tableSettings, Collections.emptyMap(), maxResults, storeSize);

        for (int i = 0; i < 3000; i++) {
            final Val[] values = new Val[]{ValString.create("Text " + i)};
...
//        dataSourceFieldsMap.put(indexField);
        ...
        for (int i = 0; i < 3000; i++) {
            ...
            final Val[] values = new Val[]{ValString.create(text)};
            tableDataFactory.add(values);
        }
...
//        dataSourceFieldsMap.put(indexField);
        ...
        for (int i = 0; i < 3000; i++) {
            final String text = "Text " + (int) (Math.random() * 100);
            final Val[] values = new Val[]{null, ValString.create(text)};
            tableDataFactory.add(values);
        }
