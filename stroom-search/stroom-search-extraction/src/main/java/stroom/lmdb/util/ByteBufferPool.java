...
public class ByteBufferPool implements Clearable, HasHealthCheck {
    ...
    public void context(final int minCapacity, final Consumer<ByteBuffer> consumer) {
        try (final PooledByteBuffer pooledKeyBuffer = getPooledByteBuffer(minCapacity)) {
            consumer.accept(pooledKeyBuffer.getByteBuffer());
        }
    }

    public <R> R contextResult(final int minCapacity, final Function<ByteBuffer, R> function) {
        R result;
        try (final PooledByteBuffer pooledKeyBuffer = getPooledByteBuffer(minCapacity)) {
            result = function.apply(pooledKeyBuffer.getByteBuffer());
        }
        return result;
    }
